...
def main():
    ...
    network_path = r"\\192.168.100.147\audio\silence-manipulator\silence-manipulator"
    st.write("Upload the files here:")
    st.text(network_path)

    # File drop input
    with st.expander("FIles drop"):
    ...
    # Folder input
    folder_name = st.text_input("Select a folder from the audio directory:", value = "silence-manipulator/")
    ...
    if st.button("Calculate Silence",type="primary", use_container_width=True):
        audio_files = []
        st.write("")
